...
// originals; `use fx_extras::*` in `audio/dsp/mod.rs` keeps the
// existing `use fx::*;` import sites unchanged.
//
// Structs in this file: Flanger, Limiter, Svf, CombRes, Tilt, Transient,
// Exciter, Multitap, RevDelay.  All allocation-free in `process()`;
// ring buffers that are too large for the stack are
// `Box<[f32; N]>`-allocated once at construction.
//
// The glitch / time-domain performance FX (TapeStop, Stutter,

        ...
        "properties": {
            ...
            "sample": {
                ...
                "properties": {
                    ...
                    "mic_blend":           { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "Multi-mic blend.  Drives the standard SFZ CC#1 (mod wheel) crossfade so multi-mic packs blend across positions (close / room / ambient).  Regions without xfin/xfout opcodes pass through at unity, so non-multi-mic SFZ files are unaffected." },
            ...
            "fx": {
                ...
                "properties": {
                    ...
                    "vinyl_transient":     { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "vinyl start/stop transient 0..1 — 0=at speed (steady-state colour), 1=deck stopped (silent). Automate 0→1 for brake, 1→0 for spin-up." },
                    ...
                    "resbank_root":        { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "resonator-bank root pitch 0..1 → MIDI 24..96 (C1..C7) — all six tuned BPFs are based on this" },
                    "resbank_chord":       { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "resonator-bank chord preset 0..1 → quantised to 6 chords: 0=minor7, 1=major triad spread, 2=dom9, 3=open fifths, 4=octave stack, 5=cluster" },
                    "resbank_resonance":   { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "resonator-bank Q 0..1 → 1..50 log-mapped (high Q=singing pitches, low=muted ringing)" },
                    "resbank_mix":         { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "resonator-bank wet/dry; 0=bypass" },
                    "tape_echo_time":      { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "tape-echo delay time 0..1 → 25..1500 ms log-mapped (slap-back at 0, dub-rhythm at 1; free-running, not BPM-synced)" },
                    "tape_echo_feedback":  { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "tape-echo regen 0..1 → 0..0.95 — stays sub-unity even at full knob to avoid runaway with AGE saturation engaged" },
                    "tape_echo_age":       { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "tape-echo character — single knob folds wow/flutter depth + saturation drive + HF rolloff together. 0=clean digital, 1=ruined cassette" },
                    "tape_echo_mix":       { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "tape-echo wet/dry; 0=bypass" },
                    "mb_low_thresh":       { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "multiband-compressor low-band threshold (linear amplitude). 1.0=no compression. Lower = more aggressive ducking when the band envelope rises above the level" },
                    "mb_mid_thresh":       { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "multiband-compressor mid-band threshold. Mid is computed subtractively from the LP outputs (LP_2500 minus LP_250)." },
                    "mb_high_thresh":      { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "multiband-compressor high-band threshold (band above ~2.5 kHz)" },
                    "mb_mix":              { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "multiband-compressor wet/dry; 0=bypass" },
                    "grain_delay":         { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "grain-delay base time 0..1 → 50..1000 ms log-mapped (centre of the grain cloud)" },
                    "grain_size":          { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "grain length 0..1 → 20..200 ms (short=chorus/verb cloud, long=smeared delay tap)" },
                    "grain_scatter":       { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "grain pitch + position jitter 0..1 — 0=aligned grains (chorus around baseline); 1=wide ±1 octave pitch jitter + ±50% position scatter" },
                    "grain_mix":           { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "grain-delay wet/dry; 0=bypass" },
                    "spec_stft":           { "type": "boolean", "description": "spectral-gate STFT mode flag — false=8-band BPF approximation (V1, default), true=true STFT (windowed FFT → per-bin gate → IFFT → overlap-add). STFT gives finer frequency resolution but adds ~21 ms latency at 48 kHz." },
                    "spec_thresh":         { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "spectral-gate per-bin (STFT) or per-band (BPF) threshold (linear amplitude). Bands/bins whose envelope falls below get gated toward silence; above stay open. 0=transparent" },
                    "spec_release":        { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "spectral-gate release 0..1 → 10..2000 ms log-mapped (long values freeze low-level resonance after a transient — spectral-hold effect)" },
                    "spec_tilt":           { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "spectral-gate threshold tilt across the spectrum. 0.5=uniform; <0.5=high bands gate more aggressively; >0.5=low bands gate more aggressively" },
                    "spec_mix":            { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "spectral-gate wet/dry; 0=bypass" },
                    "plate_size":          { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "plate-reverb tank time/decay scale 0..1 (longer tail at higher values; figure-eight Dattorro tank, distinct from FxReverb's Schroeder stack)" },
                    "plate_damping":       { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "plate-reverb HF damping 0..1 (0=bright/metallic, 1=very dark)" },
                    "plate_diffusion":     { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "plate-reverb input diffusion 0..1 (low=thin/discrete reflections, high=dense/smooth)" },
                    "plate_mix":           { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "plate-reverb wet/dry; 0=bypass" },
                    "tg_pattern":          { "type": "integer", "minimum": 0, "maximum": 65535, "description": "trance-gate 16-cell pattern as u16 bitmask — bit 0 = cell 0, bit 15 = cell 15. e.g. 0xAAAA (43690)=alternating; 0xFFFF (65535)=all open; 0xCCCC (52428)=pairs. Pattern repeats per bar at the rate selector's cells-per-bar count." },
                    "tg_rate":             { "type": "integer", "minimum": 0, "maximum": 3, "description": "trance-gate cell rate: 0=1/4 (4 cells/bar), 1=1/8 (8 cells/bar), 2=1/16 (16 cells/bar), 3=1/32 (32 cells/bar — pattern repeats twice per bar)" },
                    "tg_smooth":           { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "trance-gate per-cell-edge ramp 0..1 → 0.5..50 ms (low=tight click-free chops, high=audible swell)" },
                    "tg_mix":              { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "trance-gate wet/dry; 0=bypass, 1=fully gated" },
                    "wf_drive":            { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "wavefolder pre-fold gain 0..1 → 1..10x. drive is what pushes the signal above the fold threshold; multiplies harmonics. distinct from drive/saturation/clip (those compress; fold reflects)" },
                    "wf_bias":             { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "wavefolder DC offset 0..1 (0.5=centred symmetric fold; off-centre produces asymmetric fold with more even-order harmonics)" },
                    "wf_symmetry":         { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "wavefolder fold-curve blend 0..1 (0=pure sine fold/Buchla character, 1=pure triangle fold/Serge character; mid=hybrid)" },
                    "wf_mix":              { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "wavefolder wet/dry; 0=bypass" },
                    ...
                    "conv_reverb_shimmer":  { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "shimmer feedback 0..1 — folds the wet pitch-shifted +12 semitones (one octave up) back into the convolution input. Classic ambient-shimmer ladder." },

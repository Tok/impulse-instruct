// ─── ui/header.rs ─────────────────────────────────────────────────────────────
// Transport / status header strip + the combined log-and-scope panel.
// The File / Edit / View / Help menu bar lives in `header_menu.rs`;
// both modules add methods to the same `impl ImpulseApp` block via
// separate blocks.  Extracted when header.rs hit the 1000-line cap.

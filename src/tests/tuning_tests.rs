...
// third" (the classic tuning-system smoke test).  Also spot-checks the
// `TuningSystem::from_u8` fallback — any unknown integer must land on
// `TwelveTet` so corrupt session.json files don't silently detune the
// whole mix.

use crate::audio::dsp::{TuningSystem, midi_to_hz, midi_to_hz_tuned};

const C4_HZ: f32 = 261.626;
const A4_HZ: f32 = 440.0;

#[test]
fn twelve_tet_variant_matches_plain_midi_to_hz() {
    // TwelveTet branch must equal the plain `midi_to_hz` helper, which is
    // the reference the other tuning systems cross-compare against.
    for note in [48_u8, 60, 69, 72, 84] {
        assert!(
            (midi_to_hz_tuned(note, TuningSystem::TwelveTet) - midi_to_hz(note)).abs() < 1e-3,
            "TwelveTet must equal midi_to_hz at MIDI {note}",
        );
    }
}

#[test]
fn from_u8_unknown_values_fall_back_to_twelve_tet() {
    // Any integer outside 0..=3 must round-trip to TwelveTet — the
    // serde boundary for `FxState::tuning` can technically deliver
    // garbage from a hand-edited session.json.
    assert_eq!(TuningSystem::from_u8(0), TuningSystem::TwelveTet);
    assert_eq!(TuningSystem::from_u8(99), TuningSystem::TwelveTet);
    assert_eq!(TuningSystem::from_u8(255), TuningSystem::TwelveTet);
    assert!((midi_to_hz_tuned(69, TuningSystem::from_u8(99)) - A4_HZ).abs() < 1e-2);
}

#[test]
fn just_intonation_c4_anchors_at_standard_c() {
    // C4 (MIDI 60) is the unison in every tuning — degree=0, ratio=1.0.
    let c4 = midi_to_hz_tuned(60, TuningSystem::JustIntonation);
    assert!(
        (c4 - C4_HZ).abs() < 0.5,
...
fn just_intonation_octave_doubles_frequency() {
    ...
    let c4 = midi_to_hz_tuned(60, TuningSystem::JustIntonation);
    let c5 = midi_to_hz_tuned(72, TuningSystem::JustIntonation);
    assert!(
        (c5 / c4 - 2.0).abs() < 1e-3,
...
fn just_intonation_major_third_differs_from_equal_tempered() {
    ...
    let e4_ji = midi_to_hz_tuned(64, TuningSystem::JustIntonation);
    let e4_12tet = midi_to_hz_tuned(64, TuningSystem::TwelveTet);
...
fn slendro_is_five_tone_equal() {
    ...
    let s0 = midi_to_hz_tuned(60, TuningSystem::Slendro);
    let s1 = midi_to_hz_tuned(61, TuningSystem::Slendro);
    let s5 = midi_to_hz_tuned(65, TuningSystem::Slendro);
...
fn slendro_differs_from_12_tet_above_the_fifth_step() {
    ...
    // 12-TET.
    let slendro = midi_to_hz_tuned(65, TuningSystem::Slendro);
    let twelve_tet = midi_to_hz_tuned(65, TuningSystem::TwelveTet);
    assert!(
...
fn pelog_is_seven_tone_per_octave() {
    ...
    let p0 = midi_to_hz_tuned(60, TuningSystem::Pelog);
    let p7 = midi_to_hz_tuned(67, TuningSystem::Pelog);
    assert!(
        (p7 / p0 - 2.0).abs() < 1e-3,
...
fn pelog_intervals_are_non_equal() {
    ...
    let r1 = midi_to_hz_tuned(61, TuningSystem::Pelog) / midi_to_hz_tuned(60, TuningSystem::Pelog);
    let r2 = midi_to_hz_tuned(64, TuningSystem::Pelog) / midi_to_hz_tuned(63, TuningSystem::Pelog);
    assert!(
        (r1 - r2).abs() > 1e-3,

...
mod llm_apply_rack_tests {
    ...
    fn rack_connect_creates_cable() {
        ...
        // Connect bitcrush → delay (a cable that likely doesn't exist by default)
        let update = serde_json::json!({
        ...
        assert!(
            s.rack.cables.len() > initial_cables || {
                // Cable may already exist in default rack — check it's there
                let bc_id = s
                    .rack
                    .modules
                    .iter()
    ...
    fn rack_disconnect_removes_cable() {
        let s = AppState::default();
        // First find a cable that exists by default and disconnect it
        // Default rack has waveshaper → reverb as first in chain
        let update = serde_json::json!({
    ...
    fn rack_connect_does_not_duplicate() {
        let s = AppState::default();
        // The default rack already has cables; try to duplicate an existing one
        // waveshaper → reverb should exist
        let initial = s.rack.cables.len();
        let update = serde_json::json!({
...
mod llm_apply_step_array_tests {
    ...
    fn index_list_activates_positions() {
        ...
        // Fewer than 16 elements → index list mode
        let arr = vec![
            serde_json::json!(0),
            serde_json::json!(4),
            serde_json::json!(8),
    ...
    fn max_write_limits_writes() {
        ...
        let arr: Vec<serde_json::Value> = vec![];
        // max_write = 16 — items beyond 16 should be untouched
        apply_llm_step_array(&arr, &mut items, 16, |s, a| s.active = a);
        assert!(
            items[20].active,

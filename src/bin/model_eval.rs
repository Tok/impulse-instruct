...
use impulse_instruct::state::{AppState, DrumVoice, apply_llm_update};
...
struct Check {
    name: &'static str,
    eval: Box<dyn Fn(&AppState) -> CheckResult + Send + Sync>,
}
...
impl Check {
    ...
    fn bpm(name: &'static str, lo: f32, hi: f32) -> Self {
...
fn build_style_specs() -> Vec<StyleSpec> {
    vec![
        StyleSpec {
            id: "acid_classic",
        ...
        StyleSpec {
            id: "ambient_techno",
            prompt: "FULL RESET to ambient techno — machines dreaming, very long reverb, slow filter sweeps",
            checks: vec![
        ...
        StyleSpec {
            id: "dark_ambient",
            prompt: "FULL RESET to dark ambient — no kick, slow bass drone, maximum reverb, cavernous space",
            checks: vec![
                ...
                Check::new("no kick", |s| {
                    ...
                    if kicks == 0 {
                        ...
                        CheckResult::Fail(format!(
                            "{kicks} kick steps active — dark ambient should have none"
                        ))
                    }
                }),
            ],
        ...
        StyleSpec {
            id: "drum_and_bass",
            prompt: "FULL RESET to drum and bass — fast breakbeat energy, deep sub bass, no 4-on-floor",
            checks: vec![
                Check::bpm("bpm 165–185", 165.0, 185.0),
                Check::ge("bass volume ≥0.75", |s| s.bass.volume, 0.75),
                Check::new("no 4-on-floor kick (DnB has breakbeats)", |s| {
                    ...
                    if !is_4otf {
                        ...
                        CheckResult::Fail(
                            "pure 4-on-floor kick — DnB should have breakbeat feel".into(),
                        )
                    }
                }),
            ],
        ...
        StyleSpec {
            id: "jungle",
            prompt: "FULL RESET to jungle — amen break energy, complex breakbeats, deep sparse sub bass",
            checks: vec![
                Check::bpm("bpm 155–180", 155.0, 180.0),
                Check::ge(
